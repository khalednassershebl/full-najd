...
@section('title', 'تعديل الخدمة | لوحة الإدارة')
...
@section('content')
<form action="{{ route('admin.services.update', $service) }}" method="POST" id="service-form">
    @csrf
    @method('PUT')
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-1">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $service->title) }}" required placeholder="عنوان الخدمة">
                            @error('title')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-6 mb-1">
                            <label class="form-label" for="sort_order">ترتيب العرض</label>
                            <input type="number" class="form-control" id="sort_order" name="sort_order" value="{{ old('sort_order', $service->sort_order) }}" min="0" placeholder="0">
                            @error('sort_order')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-12 mb-1">
                            <label class="form-label" for="description">الوصف</label>
                            <textarea class="form-control" id="description" name="description" rows="4" placeholder="وصف الخدمة">{{ old('description', $service->description) }}</textarea>
                            @error('description')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-12 mb-1">
                            <label class="form-label" for="icon_svg">أيقونة SVG (اختياري)</label>
                            <textarea class="form-control font-monospace" id="icon_svg" name="icon_svg" rows="6" placeholder="<svg>...</svg>">{{ old('icon_svg', $service->icon_svg) }}</textarea>
                            <p class="text-muted small mb-0">أدخل كود SVG الكامل للأيقونة.</p>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    @php $relatedIds = old('related_services', $service->relatedServices->pluck('id')->toArray()); @endphp
                    @if(($allServices ?? collect())->isEmpty())
                        <p class="text-muted mb-0">لا توجد خدمات أخرى. أضف خدمات أخرى أولاً لربطها هنا.</p>
                    @else
                        <div class="row">
                    ...
                    @php $relatedPageIds = old('related_service_pages', $service->relatedServicePages->pluck('id')->toArray()); @endphp
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="text-muted small mb-0">المحتوى المعروض في صفحة الخدمة المفردة. يمكنك تعديله من المحرر أدناه.</p>
                </div>
                <div class="card-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var initialContent = {!! json_encode(old('page_content', $service->page_content)) !!} || '';
