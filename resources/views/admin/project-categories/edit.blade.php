...
@section('title', 'تعديل تصنيف المشاريع | لوحة الإدارة')
@section('page-title', 'تعديل تصنيف المشاريع')
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-1">
                            ...
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $projectCategory->name) }}" required placeholder="مثال: برمجة المواقع">
                            @error('name')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-4 mb-1">
                            <label class="form-label" for="slug">الرابط (Slug)</label>
                            <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $projectCategory->slug) }}" placeholder="يُولد تلقائياً من الاسم">
                            @error('slug')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-2 mb-1">
                            <label class="form-label" for="sort_order">ترتيب العرض</label>
                            <input type="number" class="form-control" id="sort_order" name="sort_order" value="{{ old('sort_order', $projectCategory->sort_order) }}" min="0">
