...
@section('title', 'تعديل المنتج | لوحة الإدارة')
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-8 mb-1">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $product->title) }}" required placeholder="عنوان المنتج">
                            @error('title')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-4 mb-1">
                            <label class="form-label" for="slug">الرابط (Slug)</label>
                            <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $product->slug) }}" placeholder="يُولد تلقائياً من العنوان">
                        ...
                        <div class="col-md-3 mb-1">
                            <div class="form-check mt-md-4 pt-1">
                                ...
                                <input type="checkbox" class="form-check-input" name="is_published" id="is_published" value="1" {{ old('is_published', $product->is_published) ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_published">منشور</label>
                            </div>
                        </div>
                        <div class="col-12 mb-1">
                            <label class="form-label" for="excerpt">المقتطف</label>
                            <textarea class="form-control" id="excerpt" name="excerpt" rows="2" placeholder="نبذة قصيرة تظهر في البطاقة">{{ old('excerpt', $product->excerpt) }}</textarea>
                        ...
                        <div class="col-12 mb-1">
                            ...
                            <input type="url" class="form-control" id="demo_url" name="demo_url" value="{{ old('demo_url', $product->demo_url) }}" placeholder="https://...">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var initialContent = {!! json_encode(old('content', $product->content)) !!} || '';
