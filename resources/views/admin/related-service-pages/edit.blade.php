...
@section('title', 'تعديل صفحة الخدمة ذات الصلة | لوحة الإدارة')
@section('page-title', 'تعديل صفحة الخدمة ذات الصلة')
...
@section('content')
<form action="{{ route('admin.related-service-pages.update', $relatedServicePage) }}" method="POST" id="related-page-form">
    @csrf
    @method('PUT')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="text-muted small mb-0">الرابط: /services/related/{{ $relatedServicePage->slug }}</p>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-1">
                            <label class="form-label" for="title">العنوان <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $relatedServicePage->title) }}" required>
                            @error('title')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-3 mb-1">
                            <label class="form-label" for="slug">الرابط (slug)</label>
                            <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $relatedServicePage->slug) }}">
                            @error('slug')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-md-3 mb-1">
                            <label class="form-label" for="sort_order">ترتيب العرض</label>
                            <input type="number" class="form-control" id="sort_order" name="sort_order" value="{{ old('sort_order', $relatedServicePage->sort_order) }}" min="0">
                        ...
                        <div class="col-12 mb-1">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="2">{{ old('description', $relatedServicePage->description) }}</textarea>
                            @error('description')<span class="text-danger small">{{ $message }}</span>@enderror
                        </div>
                        <div class="col-12 mb-1">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $relatedServicePage->is_active) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var initial = {!! json_encode(old('page_content', $relatedServicePage->page_content)) !!} || '';
