...
$(function () {
  ...
  if (dt_basic_table.length) {
    var dt_basic = dt_basic_table.DataTable({
      ajax: assetPath + "data/students-data.json",
      columns: [
        { data: "id" },
      ...
      columnDefs: [
          ...
          render: function (data, type, full, meta) {
            return `
              <div class="num-badge d-flex align-items-center gap-2">
                <span class="num-badge-data"><span class="le">PTS</span>${data}</span>
                <a href="#" class="table-item-icon add-points-btn" data-id="${full.id}" data-points="${data}" data-name="${full.student_name || ''}" title="إضافة نقاط">
          ...
          render: function (data, type, full, meta) {
            return `
              <div class="num-badge d-flex align-items-center gap-2">
                ...
                <a href="#" class="table-item-icon add-balance-btn" data-id="${full.id}" data-balance="${data}" data-name="${full.student_name || ''}" title="تعديل الرصيد">
                  ${feather.icons["edit"].toSvg({ class: "font-small-4" })}
                </a>
              </div>
            `;
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $status = {
              1: { title: "جديد", class: "status-new" },
              2: { title: "مٌفعل", class: "status-active" },
              3: { title: "محظور", class: "status-ban" },
            };
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $user_img = full["avatar"],
              $name = full["student_name"],
              $post = full["student_id"];
            var stateNum = full["status"];
            var states = ["status-active"];
            var $state = states[stateNum] || "status-active";
            
            if ($user_img && $user_img !== '') {
              // For Avatar image - use full URL from backend
              var $output =
                '<img src="' +
                $user_img +
                '" alt="Avatar" width="32" height="32" style="object-fit: cover;">';
            } else {
              // For Avatar badge
            ...
            var colorClass =
              ($user_img === "" || !$user_img) ? " bg-light-" + $state + " " : "";
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var editUrl = '/admin/students/' + full.id + '/edit';
            var studentName = full.student_name || '';
            return (
              '<div class="action-container">' +
              '<a href="' + editUrl + '" class="table-item-icon table-item-edit" title="تعديل">' +
              feather.icons["edit"].toSvg({ class: "font-small-4" }) +
              "</a>" +
              '<a href="#" class="table-item-icon table-item-devices" data-id="' + full.id + '" data-name="' + studentName + '" title="الأجهزة">' +
              feather.icons["monitor"].toSvg({ class: "font-small-4" }) +
              "</a>" +
              '<a href="#" class="table-item-icon table-item-delete" data-id="' + full.id + '" data-name="' + studentName + '" title="حذف">' +
        ...
        { visible: false, targets: [0, 1, 4, 6, 9, 13] },
      ],
      order: [[1, "desc"]],
    ...
    $(dt_basic_table).on('click', '.table-item-delete', function(e) {
      ...
      if (studentId && confirm('هل أنت متأكد من حذف الطالب "' + studentName + '"؟')) {
