...
$(function () {
  ...
  if (dt_basic_table.length) {
    var dt_basic = dt_basic_table.DataTable({
      ajax: assetPath + "data/groups-data.json",
      columns: [
        { data: "id" },
        { data: "id" }, // used for sorting so will hide this column
        { data: "group_name" },
        { data: "stage_name" },
        { data: "students" }, // Combined: students and months data
        { data: "start_date" },
        { data: "" },
      ],
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var monthsCount = full.months || full.groups || 0; // Use months if available, otherwise groups data
            var studentsUrl = '/admin/groups/' + full.id + '/students';
            return `
              <div style="display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
                <div class="num-badge" style="display: flex; align-items: center; gap: 5px;">
                  ...
                  <span style="font-size: 11px; color: #6e6b7b;">طلاب</span>
                  <a href="${studentsUrl}" class="table-item-icon" title="عرض الطلاب" style="margin-right: 3px;">
                    ${feather.icons["eye"].toSvg({ class: "font-small-4" })}
                  </a>
                </div>
                ${monthsCount > 0 ? `
                <div class="num-badge" style="display: flex; align-items: center; gap: 5px;">
                  <span class="num-badge-data">${monthsCount}</span>
                  <span style="font-size: 11px; color: #6e6b7b;">شهور</span>
                </div>
                ` : ''}
              </div>
            `;
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var editUrl = '/admin/groups/' + full.id + '/edit';
            var groupName = full.group_name || '';
            return (
              '<div class="action-container">' +
              '<a href="' + editUrl + '" class="table-item-icon table-item-edit" title="تعديل" style="margin-left: 5px;">' +
              feather.icons["edit"].toSvg({ class: "font-small-4" }) +
              "</a>" +
              '<a href="#" class="table-item-icon table-item-delete" data-id="' + full.id + '" data-name="' + groupName + '" title="حذف" style="margin-left: 5px;">' +
    ...
    $(dt_basic_table).on('click', '.table-item-delete', function(e) {
      ...
      if (groupId && confirm('هل أنت متأكد من حذف المجموعة "' + groupName + '"؟')) {
        // Create a form to submit DELETE request
        var form = $('<form>', {
          'method': 'POST',
          'action': '/admin/groups/' + groupId
        });
